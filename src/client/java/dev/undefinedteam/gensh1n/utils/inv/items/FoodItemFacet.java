...
//                    Comparator.comparingInt(it -> it.getItemStack().getCount()),
    ...
    public List<Pair<ItemFunction, Integer>> getProvidedItemFunctions() {
        ...
        if (getItemStack().getItem().getFoodComponent() != null) {
            functions.add(new Pair<>(ItemFunction.FOOD, (int)(getItemStack().getCount() * getItemStack().getItem().getFoodComponent().getSaturationModifier())));
        }
        return functions;
    }
