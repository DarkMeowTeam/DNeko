...
public class Speed extends Module {
    ...
    public void onUpdate(PlayerTickEvent event) {
        if (this.mc.player != null) {
            ...
            if (boost.get()) {
                if (velocity.y < 0.1 && velocity.y > 0.01) {
                    this.mc.player.setVelocity(new Vec3d(velocity.x * 1.005, velocity.y, velocity.z * 1.005));
                }
                if (velocity.y < 0.005 && velocity.y > 0.0) {
                    this.mc.player.setVelocity(new Vec3d(velocity.x * 1.005, velocity.y, velocity.z * 1.005));
                }
                if (velocity.y < 0.001 && velocity.y > -0.03) {
                    if (this.mc.player.hasStatusEffect(StatusEffects.SPEED)) {
                        this.mc.player.setVelocity(new Vec3d(velocity.x * 1.005, velocity.y, velocity.z * 1.005));
                    } else {
                        this.mc.player.setVelocity(new Vec3d(velocity.x * 1.002, velocity.y, velocity.z * 1.002));
                    }
                }
            }
        }
