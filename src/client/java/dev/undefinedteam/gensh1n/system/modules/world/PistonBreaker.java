...
import net.darkmeow.dneko.inject.accessor.AccessorPlayerMoveC2SPacket;
...
public class PistonBreaker extends Module {
    ...
    private void onMouse(MouseButtonEvent e) {
        if (e.action.equals(KeyAction.Press) && mc.currentScreen == null) {
            ...
            if (selecting) {
                var left = e.button == ((AccessorKeyBinding) mc.options.attackKey).getBoundKey().getCode();
                var right = e.button == ((AccessorKeyBinding) mc.options.useKey).getBoundKey().getCode();
    ...
    private void onPacket(PacketEvent e) {
        if (e.packet instanceof PlayerMoveC2SPacket p) {
            ((AccessorPlayerMoveC2SPacket) p).setYaw(onModifyLookYaw(((AccessorPlayerMoveC2SPacket) p).getYaw()));
            ((AccessorPlayerMoveC2SPacket) p).setPitch(onModifyLookPitch(((AccessorPlayerMoveC2SPacket) p).getPitch()));
        }
    }
